...
namespace StylizedWaterShader
{
    public class StylizedWaterCore : Editor
    {
        ...
        public const string INSTALLED_VERSION = "2.1.9";
        public const string MIN_UNITY_VERSION = "2019.4";
        public const string MAX_UNITY_VERSION = "2023.2";
            ...
            static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
            {
                foreach (string str in importedAssets)
                {
                    if (str.Contains("StylizedWaterCore.cs"))
                    {
                        ...
                        if (!compatibleVersion || outdatedVersion || alphaVersion)
                        {
    ...
    public class SWS_Window : EditorWindow
    {
        ...
        void DrawTabs()
        {
            ...
            Texture2D dot = null;
            if (StylizedWaterCore.compatibleVersion && !StylizedWaterCore.alphaVersion && !StylizedWaterCore.outdatedVersion) dot = SmallGreenDot;
            else if (StylizedWaterCore.alphaVersion || !StylizedWaterCore.IS_UPDATED) dot = SmallOrangeDot;
            else if (!StylizedWaterCore.compatibleVersion || StylizedWaterCore.outdatedVersion) dot = SmallRedDot;
        ...
        void DrawInstallation()
        {
            ...
            if (StylizedWaterCore.compatibleVersion && !StylizedWaterCore.alphaVersion && !StylizedWaterCore.outdatedVersion)
            {
                GUI.contentColor = Color.green;
            ...
            else
            {
                ...
                EditorGUILayout.LabelField("Incompatible", EditorStyles.boldLabel);
                GUI.contentColor = defaultColor;
            }
            ...
            if (StylizedWaterCore.compatibleVersion == false || StylizedWaterCore.alphaVersion || StylizedWaterCore.outdatedVersion)
            {
                GUI.contentColor = Color.red;
                EditorGUILayout.LabelField("This version of Unity is not supported. Errors or issue may be present.", EditorStyles.boldLabel);

                if (StylizedWaterCore.outdatedVersion)
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.LabelField("Please upgrade to at least Unity " + StylizedWaterCore.MIN_UNITY_VERSION);
                }

                if (StylizedWaterCore.compatibleVersion == false)
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.LabelField("The latest compatible version is Unity " + StylizedWaterCore.MAX_UNITY_VERSION);
                }
        ...
        void DrawSupport()
        {
            ...
            //Buttons box
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("<b><size=12>Discord</size></b>\n<i>Access support</i>", Button))
            {
                Application.OpenURL(StylizedWaterCore.DISCORD_INVITE_URL);
            }
